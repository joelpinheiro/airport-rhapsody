 ...
 *  \author António Rui Borges - September 2013
 */
...
void printSuperBlock (void *buf)
{
  ...
  printf ("Cluster-to-inode table metadata\n");
  printf ("   Physical number of the block where the cluster-to-inode table starts  = ");
  if (p_sb->ciutable_start == NULL_BLOCK)
     printf ("(nil)\n");
     else printf ("%"PRIu32"\n", p_sb->ciutable_start);
  printf ("   Number of blocks that the cluster-to-inode table comprises  = %"PRIu32"\n", p_sb->ciutable_size);

  /* data zone metadata */

  printf ("Data zone metadata\n");
  ...
  printf("\n");
  printf ("   Physical number of the block where the bitmap table to data clusters starts  = ");
  if (p_sb->fctable_start == NULL_BLOCK)
     printf ("(nil)\n");
     else printf ("%"PRIu32"\n", p_sb->fctable_start);
  printf ("   Number of blocks that the bitmap table to data clusters comprises  = %"PRIu32"\n", p_sb->fctable_size);
  printf ("   Search point index for the bitmap table envisaged as an array of bits (circular parsing)  = %"PRIu32"\n",
          p_sb->fctable_pos);
}
