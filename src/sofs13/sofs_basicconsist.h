 ...
 *
 *  \brief Set of operations to check the basic consistency of the file system SOFS13 internal data structures.
 *
 *
 ...
 *
 *  \author António Rui Borges - September 2010 / October 2013
 *
...
/** \brief base value for inconsistency errors */
#define ERROR_BASE           512
...
/** \brief number of free data clusters is overall inconsistent */
...
#define ERELPATH             526
/** \brief free inode in clean state is inconsistent */
#define EFCININVAL           527
/** \brief free inode in dirty state is inconsistent */
#define EFDININVAL           528
/** \brief mapping association of the data cluster is invalid */
#define EDCMINVAL            529
 ...
 *  computed through inspection of the two free data cluster caches and the bitmap table to free data clusters must
 *  match the value stored in the related field of the superblock. The contents of the two caches are also checked: the
 *  references stored there must be legal.
 *
 *  \param p_sb pointer to a buffer where the superblock data is stored
 *
 *  \return <tt>0 (zero)</tt>, on success
...
/**
 *  \brief Quick check of the list of data cluster references belonging to an inode either in use or free in the dirty state.
 *
 *  All the clusters referenced in the list must have legal logical numbers and be allocated or freed, according to the inode
 *  status, and the <tt>clucount</tt> field must have a value set in accordance to the filling of the list.
 *
 *  \param p_sb pointer to a buffer where the superblock is stored
 *  \param p_inode pointer to a buffer where the inode is stored
 *
 ...
 *
 *  The caches and the bitmap table to free data clusters are searched to check if they include the reference to
 *  the data cluster. If the reference is there, the data cluster is marked as free; otherwise, it is marked as allocated.
 *
